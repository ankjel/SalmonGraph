...
#wfmash
param_s=5000 # segment size, this should only be this small because we have a small graph, for full chromosomes set to 100000
...
param_l=$(expr "$param_s"*3) # minimum block length filter for mapping. (segments are merged to blocks, default 3*segment-length)
...
SCRATCHout=/mnt/SCRATCH/ankjelst/data/prdm9/pggb-TEST-G$param_G-k$param_k.out

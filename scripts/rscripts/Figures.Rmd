...
#make color palette
mycolors=c(colorRampPalette("gray")(1),
         colorRampPalette(brewer.pal(9,"Accent"))(7))
names(mycolors)=c(grep("pangenie",unique(time.tbl$combined),value = T),
                grep("vg",unique(time.tbl$combined),value = T))




time.tbl %>% 
  mutate(combined.variable = factor(paste(pipeline, param.value, sep = "~"))) %>% 
  ggplot(aes(y = `memory(kbytes)`, x = depth, fill = combined, group = pipeline)) + geom_col(position = "dodge") +
  scale_fill_manual(name = "Tool", values = mycolors) + theme_minimal()+
...
time.tbl  %>% 
  ggplot(aes(y = minutes, x = depth, fill = combined, group = pipeline)) + geom_col(position = "dodge") +theme_minimal()+
  scale_fill_manual(name = "Tool", values = mycolors) + 

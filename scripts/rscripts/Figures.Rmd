...
vcf.true %>%
  ...
  lims(x = c(50, 1500)) -> small


plot_grid(full, small , ncol = 1, labels = c("A", "B"), 
          rel_heights = c(1, 2), label_size = 10)
```
...
plot.df %>% 
  ...
  scale_fill_brewer(palette = "Dark2")+ 
  facet_wrap(vars(Parameter), scales = "free_x")+ 
  theme(axis.text.x = element_text(angle = 55, vjust = 0.5)) -> time



plot_grid(mem, time, ncol = 1, label_size = 10, align="v")
...
rbind(pg, vg) %>% 
  mutate(depth = as.integer(depth),
         TYPE = ifelse(TYPE == "deletion", "Deletions", "Insertions")) %>% 
  ggplot(aes(x = depth, y = precision, color = tool, group = tool)) + geom_line() + geom_point() + facet_wrap(vars(TYPE)) +
...
```
```{r}
sq <- microseq::readFasta("/mnt/SCRATCH/ankjelst/data/prdm9/PRDM9a_zf_sig_original.fasta")

sq %>% 
  mutate(len = str_length(Sequence), 
         zf = round((len/84)-3)) -> sq
```
...
full <- tibble()
for (rown in 1:nrow(sq)){
  nam <- sq$Header[rown]
  nznf <- sq$zf[rown]
  start <- seq(from = 1, to = (nznf*84), by = 84)
  end <- seq(from = 84, to = (nznf*84), by = 84)
  df <- tibble(name = nam, start = start, end = end)
  full <- rbind(full, df)
}


full %>% 
  mutate(name2 = str_extract(name, pattern = "[:alpha:]*.#[1,2]")) %>%
  ggplot(aes(xmin = start, xmax = end, y = name2, color = name2)) +
  geom_linerange(size = 2)+ theme_genes()+
  theme(legend.position = "none",
        axis.text.x = element_blank(),
